...
function mergeConfig(raw: Partial<PricingConfig> | null): PricingConfig {
  if (!raw) return { ...DEFAULT_CONFIG, rates: { ...DEFAULT_CONFIG.rates } };

  return {
    minimumFare: safeNumber(raw.minimumFare, DEFAULT_CONFIG.minimumFare),
    rates: {
      tier1: safeNumber(raw.rates?.tier1, DEFAULT_CONFIG.rates.tier1),
      tier2: safeNumber(raw.rates?.tier2, DEFAULT_CONFIG.rates.tier2),
      tier3: safeNumber(raw.rates?.tier3, DEFAULT_CONFIG.rates.tier3)
    },
...
export default function Home() {
  ...
  const quote = useMemo(() => {
    ...
    const cashTransferTotal = afterPeak + hotelSurcharge + tolls + Math.max(0, waitValue);
    ...
    return {
      ...
      hotelSurcharge,
      tolls,
      waitValue: Math.max(0, waitValue),
  ...
  const currentTotal = paymentMode === "card" ? quote.cardTotal : quote.cashTransferTotal;
  ...
  useEffect(() => {
    const storedConfig = readStorage<Partial<PricingConfig>>(STORAGE_KEYS.config, null);
    const storedUsers = readStorage<AppUser[]>(STORAGE_KEYS.users, DEFAULT_USERS);
    const storedAudits = readStorage<AuditEntry[]>(STORAGE_KEYS.audits, []);
    const storedCash = readStorage<CashEntry[]>(STORAGE_KEYS.cash, []);

    setConfig(mergeConfig(storedConfig));
    setUsers(storedUsers.length ? storedUsers : DEFAULT_USERS);
    setAudits(storedAudits);
    setCashEntries(storedCash);

    const envApiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
    const storedApiKey = window.localStorage.getItem(STORAGE_KEYS.maps);
    const apiKey = envApiKey || storedApiKey || "";

    if (!apiKey) {
      setNeedsApiKey(true);
      setMessage("Google Maps esta en modo manual. Puedes cargar tu API key para rutas reales.");
    } else {
  ...
  useEffect(() => {
    if (!hasHydrated) return;
    window.localStorage.setItem(STORAGE_KEYS.audits, JSON.stringify(audits));
  }, [audits, hasHydrated]);

  useEffect(() => {
    if (!hasHydrated) return;
    window.localStorage.setItem(STORAGE_KEYS.cash, JSON.stringify(cashEntries));
  ...
  function registerAudit(action: string, userName?: string) {
    ...
    setAudits((current) => [entry, ...current].slice(0, 300));
  }
  ...
  function activateApiKey() {
    ...
    if (!key) {
      setMessage("Pega una API key valida para activar Google Maps.");
      setMessageType("error");
      return;
    }
  ...
  async function handleCalculateRoute(event: FormEvent<HTMLFormElement>) {
    ...
    if (!mapsReady) {
      if (manualKm > 0) {
        ...
        setMessageType("success");
        registerAudit("Calculadora: calculo manual de tarifa.");
        return;
      }
    ...
    try {
      ...
      setManualKm(Number(result.distanceKm.toFixed(1)));
      setMessage("Ruta calculada correctamente.");
      setMessageType("success");
      registerAudit("Calculadora: ruta y tarifa calculadas.");
    } catch (error) {
  ...
  function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    const user = users.find((item) => item.username === loginUsername.trim());
  ...
  function updatePermission(userId: string, key: keyof PermissionSet, value: boolean) {
    ...
    registerAudit(`Permisos actualizados para ${userId}: ${key} ${value ? "habilitado" : "bloqueado"}.`);
  }
  ...
  function handleAddUser(event: FormEvent<HTMLFormElement>) {
    ...
    if (users.some((item) => item.username.toLowerCase() === username)) {
      registerAudit(`Intento de alta fallido: usuario ${username} ya existe.`);
      return;
    }
    ...
    registerAudit(`Nuevo usuario creado: ${username}.`);
  }

  function handleAddCash(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();

    if (!cashConcept.trim() || cashAmount <= 0) return;

    const entry: CashEntry = {
      id: makeId("cash"),
      createdAt: new Date().toISOString(),
      concept: cashConcept.trim(),
    ...
    registerAudit(`Caja: ${cashKind === "income" ? "ingreso" : "egreso"} ${formatCurrency(entry.amount)}.`);
  }
    ...
    <main className="app-shell">
      <header className="topbar">
        <div className="brand-box">
          ...
          <div>
            <h1>Mi Chofer</h1>
            <p>Calculadora y gestion simple</p>
          </div>
        </div>
        ...
        <section className="panel grid-two">
          <article className="card">
            ...
            <form className="form-grid" onSubmit={handleCalculateRoute}>
              <label>
                ...
                <input ref={originRef} placeholder="Ej: Libertador 1000, CABA" autoComplete="off" />
              </label>
              <label>
          ...
          <article className="card">
            ...
            <dl className="summary-list">
              ...
              <div>
                <dt>Espera</dt>
                <dd>{formatCurrency(quote.waitValue)}</dd>
              </div>
              <div>
            ...
            <form className="form-grid" onSubmit={handleConfigSave}>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfig((current) => ({ ...current, peakMultiplier: Math.max(1, Number(event.target.value) || 1) }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfig((current) => ({ ...current, cardSurcharge: Math.max(0, Number(event.target.value) || 0) / 100 }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfig((current) => ({ ...current, hotelSurcharge: Math.max(0, Number(event.target.value) || 0) / 100 }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfig((current) => ({ ...current, defaultTolls: Math.max(0, Number(event.target.value) || 0) }))
                  }
                />
              </label>
